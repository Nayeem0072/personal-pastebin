...
app.get("/image/:slug", (c) => {
  ...
  if (!row || row.privacy !== "public") {
    return c.body(svgToPng(privateSvg()));
  }
...
app.get("/meta/:slug", (c) => {
  ...
  const imageUrl = esc(`${backendOrigin}/api/og/image/${slug}.png`);
...
app.get("/meta/invite/:code", (c) => {
  ...
  const imageUrl = esc(`${backendOrigin}/api/og/image/invite/${code}.png`);
  const pageUrl = esc(dest);
