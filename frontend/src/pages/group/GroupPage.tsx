      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 20 }}>
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
            ...
            <h1 style={{ fontSize: 22, fontWeight: 700, color: "var(--color-ink)", margin: 0 }}>{group.name}</h1>
            {group.visibility === "private" && <Badge variant="default">Private</Badge>}
            {role && <Badge variant="orange">{role}</Badge>}
          </div>
          <p style={{ fontSize: 13, color: "var(--color-ink-3)", margin: "0 0 6px", fontFamily: "monospace" }}>/{group.slug}</p>
          {group.description && <p style={{ fontSize: 14, color: "var(--color-ink-2)", margin: "0 0 4px" }}>{group.description}</p>}
